...
MARKETPLACE_ACCOUNT=dev-1654062670653-36995488391894
...
# npx ts-node marketplace_create_product.ts -c $MARKETPLACE_ACCOUNT -a $SHOP_ACCOUNT -p products/fabrics-delivery.test.near/product-1.json
# npx ts-node marketplace_create_product.ts -c $MARKETPLACE_ACCOUNT -a $SHOP_ACCOUNT -p products/fabrics-delivery.test.near/product-2.json
...
npx ts-node rating_get_ratings.ts -a $MARKETPLACE_ACCOUNT -c $RATING_ACCOUNT -s $SHOP_ACCOUNT -p product-1